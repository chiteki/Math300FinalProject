...
/-
# √2 is Irrational — Lean Final Project

Main result: there are no integers a and b with b ≠ 0
...
theorem eq_imp_a_even (a b : ℤ) (h : a ^ 2 = 2 * b ^ 2) : 2 ∣ a := by
  have h1 : 2 ∣ a ^ 2 := by use b ^ 2; exact h
...
theorem eq_imp_b_even (a b : ℤ) (h : a ^ 2 = 2 * b ^ 2) : 2 ∣ b := by
  ...
  have h1 : b ^ 2 = 2 * j ^ 2 := by
    ...
    cancel 2 at h2
    exact h2.symm
  have h3 : 2 ∣ b ^ 2 := by use j ^ 2; exact h1
...
theorem natAbs_half_lt {b k : ℤ} (hb : b ≠ 0) (hk : b = 2 * k) :
  ...
  have hk0 : k ≠ 0 := by
    ...
    exact hb hk
  have hpos : 0 < Int.natAbs k := Int.natAbs_pos.mpr hk0
...
-- Same as sqrt2_irr_core but states |b| = n exactly rather than |b| ≤ n
theorem sqrt2_irr_aux (n : ℕ) :
    ∀ a b : ℤ, Int.natAbs b = n → a ^ 2 = 2 * b ^ 2 → b = 0 := by
  intro a b hbn h
  exact sqrt2_irr_core n a b (le_of_eq hbn) h
...
-- If a^2 = 2b^2 then both a and b must be zero
theorem sqrt2_irrational_both (a b : ℤ) (h : a ^ 2 = 2 * b ^ 2) : a = 0 ∧ b = 0 := by
  have hb : b = 0 := sqrt2_irrational a b h
  have ha2 : a ^ 2 = 0 :=
    calc a ^ 2 = 2 * b ^ 2 := h
      _ = 2 * (0 : ℤ) ^ 2 := by rw [hb]
      _ = 0 := by ring
  have haa : a * a = 0 := by
    ...
    rw [← this]; exact ha2
  have ha : a = 0 := by
    rw [mul_self_eq_zero] at haa
    exact haa
  exact ⟨ha, hb⟩

-- There are no integers a and b with b > 0 satisfying a^2 = 2b^2
theorem sqrt2_no_rational_rep : ¬ ∃ a b : ℤ, 0 < b ∧ a ^ 2 = 2 * b ^ 2 := by
  intro ⟨a, b, hbpos, h⟩
  exact absurd (sqrt2_irrational a b h) (ne_of_gt hbpos)

-- For any integers p and q with q nonzero, p^2 cannot equal 2q^2
theorem no_fraction_sq_two (p q : ℤ) (hq : q ≠ 0) : p ^ 2 ≠ 2 * q ^ 2 := by
  intro h
  exact hq (sqrt2_irrational p q h)
